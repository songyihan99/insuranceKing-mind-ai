...
### 섹션3 (클로징) 필수 기법
- 섹션3은 **유형별로 완전히 다른 구조**로 써라. 닻내림(풀보장 먼저)과 밀당("안 하셔도 됩니다. 다른 곳과 비교해 보세요")은 공통이고, 그 **사이 내용은 반드시 유형별로** 달라야 한다.
- 공통 구조:
  1. **닻내림**: 먼저 풀보장 고급 플랜 기준 언급 (예: "풀보장으로 구성하면 월 15만 원대입니다")
  2. **유형별 본문**: 아래 `유형별 섹션3 클로징` 중 판정 유형 1가지만 그대로 반영해 써라(다른 유형 문장 섞지 마라)
  3. **밀당**: "안 하셔도 됩니다. 다른 곳과 비교해 보세요"로 마무리
- 밀당 문장 "안 하셔도 됩니다. 다른 곳과 비교해 보세요"는 **전체 응답에서 반드시 1회만** 출력해라. 중복 금지.
...
### 유형별 섹션3 클로징 (판정 유형 1가지만 선택 — 닻내림과 밀당 사이에 배치)
- 전통주의자:
...
- "만약 이 보장이 이미 고객님 지갑에 있는데, 스스로 포기하시겠어요?"라는 방식으로 이미 소유한 것처럼 가정해서 써라.
...
- 본문 해설은 **2문장 이내**로 압축한다(위 분량 규칙 준수). fp-ment-box 멘트는 분량에서 제외.
- 섹션3은 **유형별로 완전히 다른 구조**로 써라. 닻내림(풀보장 먼저) → 유형별 본문 → 밀당("안 하셔도 …") 순서를 지켜라.
- 닻내림: 풀보장 고급 플랜 기준을 먼저 언급해라.
- 유형별 본문: 위 `유형별 섹션3 클로징` 중 판정 유형 내용만 닻내림과 밀당 **사이**에 넣어라. 다른 유형 문장 금지.
- 밀당: "안 하셔도 됩니다. 다른 곳과 비교해 보세요"로 마무리(전체 응답 **1회만**).
...
- 섹션3 클로징 이후 섹션4 fp-ment-box에서 아래 **절대 금지 표현**을 쓰지 마라.
